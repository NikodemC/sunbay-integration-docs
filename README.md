...
- Safety net: during full-snapshot reconciliation, open invoices that Sunbay expected in the snapshot but did not receive are flagged and handled per the onboarding agreement (§10). Only records with `lastModifiedAt` before the crawl's `modifiedUntil` count as expected - anything modified during the crawl belongs to the next incremental poll, not to the alarm.
